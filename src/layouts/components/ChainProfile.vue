...
<template>
  <div class="dropdown">
    <label tabindex="0" class="flex items-center">
      ...
      <div class="flex-1 w-0">
        <div
          ...
          class="capitalize whitespace-nowrap text-base font-semibold text-gray-600 dark:text-[#eee] hidden md:!block"
        >
          {{
        ...
        <div class="text-xs text-gray-500 dark:text-[#b0b7c2] whitespace-nowrap hidden md:!block">
    ...
    <div tabindex="0" class="dropdown-content -left-6 w-80 menu shadow bg-base-200 rounded-box overflow-auto">
      <!-- rest -->
      <div class="px-4 py-2 text-sm text-[#b0b7c2]" v-if="chainStore.current?.endpoints?.rest">Rest Endpoint</div>
      <div
        v-for="(item, index) in chainStore.current?.endpoints?.rest"
        class="px-4 py-2 w-full hover:bg-gray-100 dark:hover:bg-[#2a2630] cursor-pointer"
        :key="index"
        @click="changeEndpoint(item)"
      >
        <div class="flex flex-col">
          <div class="flex items-center justify-between w-full">
            <div class="text-gray-500 dark:text-[#eee] capitalize">
              {{ item.provider }}
            </div>
            <span
          ...
          <div class="text-[#b0b7c2] text-xs whitespace-nowrap">
            {{ item.address }}
          </div>
        </div>
      </div>
      ...
      <!-- rest -->
      <div class="px-4 py-2 text-sm text-[#b0b7c2]">Information</div>
      <div class="w-full">
        <div class="py-2 px-4">
          Chain Id:
          {{
